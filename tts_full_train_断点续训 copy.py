...
# 优化 CPU 线程
os.environ["OMP_NUM_THREADS"] = "25"  # 匹配 25 vCPU
...
if DEBUG_TRAIN:
    ...
    DEBUG_BATCH_SIZE = 224      # 5090 32GB：128 仅用约 20GB/50–67%，提高到 224 吃满显存与算力
    DEBUG_SAVE_STEP = 500
    DEBUG_PRINT_STEP = 25
...
config.loader_num_workers = 16   # 5090 下提高 worker，减少 GPU 等数据（原 12）
...
if hasattr(config, 'loader_prefetch_factor'):
    config.loader_prefetch_factor = 16  # 多预取几个 batch，保持 GPU 饱合
# 加载训练样本
